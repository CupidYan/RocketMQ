            ...
            public ConsumeConcurrentlyStatus consumeMessage(List<MessageExt> msgs,
                ...
                ServerMQTT server;
        			 try {
        				...
        				MqttClient client = server.getClient();
        				MqttTopic topic;
        				topic = client.getTopic(new String(msg.getTopic()));
        		
        				server.setMessage( new MqttMessage());
        				server.getMessage().setQos(1);
        				server.getMessage().setRetained(true);
        				server.getMessage().setPayload(new String(msg.getBody()).getBytes());
        				server.publish(topic,server.getMessage());
